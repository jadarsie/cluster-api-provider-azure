...
func getPublicIPAddressesClient(subscriptionID, baseURI string, authorizer autorest.Authorizer) network.PublicIPAddressesClient {
	publicIPsClient := network.NewPublicIPAddressesClient(subscriptionID)
	publicIPsClient.BaseURI = baseURI
...
func NewService(scope *scope.ClusterScope) *Service {
	return &Service{
		Client: getPublicIPAddressesClient(scope.SubscriptionID, scope.BaseURI, scope.Authorizer),
		Scope:  scope,
	}
}
